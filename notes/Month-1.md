...
Month 1 · Week 3 · Weekend 1 — 模板固化

## 今日核心目标

把本周学的工程化配置固化成可复用模板，新项目 3 分钟能启动
...
区分"项目核心文件"和"工具生成的临时文件"：

- 核心文件：提交 Git，团队共享
- 临时文件：加入 .gitignore，本地生成
...
├── pyrightconfig.json      # 类型检查配置（放根目录！）
├── uv.lock                 # 依赖版本锁定
...
- pyrightconfig.json 放错位置：放在 src/ 下，根目录运行 pyright 找不到配置。必须放项目根目录
- .gitignore 不完整：忘记排除 .pytest_cache/ 和 .ruff_cache/，导致缓存文件被提交
- README 只有命令没有说明：陌生人看不懂这个项目是干什么的
...
│                    ▼                        │
│         ┌──────────────────┐               │
│         │ python-app-template │ ◀── 复制这个 │
│         │ （工程化基础模板）   │     开始新项目│
│         └──────────────────┘               │
└─────────────────────────────────────────────┘
```

模板是"地基"，Month 2 开始的 LLM Client、RAG 系统都基于这个结构
...
- README 四个章节：简介、环境、快速开始、开发命令
- 临时文件：.venv、__pycache__、各种 cache
- 配置文件放根目录

**💎 核心区（必须内化）**

- README 回答三个问题：是什么、怎么跑、有啥命令
- 区分"提交 Git"和"不提交"的原则：核心配置提交，生成物不提交
- pyrightconfig.json 必须放根目录
...
Month 1 Week 4 - CLI 综合项目（并发优化）-并发优化 - ThreadPoolExecutor

## 今日核心目标

用线程池让文件批量处理从串行变并发，实现可测量的性能提升。
...
真实场景中，文件处理、数据清洗、批量调用 API 都需要并发能力。不会并发 = 只能写玩具代码。
...
**串行**：一个接一个排队执行，CPU 大部分时间在等 IO。

**并发**：多个任务同时执行，利用 IO 等待时间处理其他任务。

### 为什么文件 IO 用线程池而不是 async？

Python 标准文件操作是**阻塞的**——调用 `open().read()` 时，程序会停在那里等磁盘。

async 的优势在于**非阻塞 IO**（如网络请求），对阻塞的文件操作帮不上忙。

线程池的做法是：开多个线程，每个线程各自阻塞等待自己的文件，互不影响。

### 类比

- **串行** = 一个窗口办 10 笔业务，排队 10 分钟
- **线程池** = 开 5 个窗口同时办，约 2 分钟
...
# 现在 task(file) 等于 count_one(file, count_lines)
```
...
- **耗时计算是批次思维，不是除法**：10 任务 / 4 workers ≠ 2.5 秒，而是 ceil(10/4) = 3 批 = 3 秒。任务不能切成小数。

- **try-except 位置决定失败行为**：放循环外 = 一个失败全停；放循环内 = 单个失败不影响其他。批量处理通常要后者。

- **max_workers 不是越大越好**：开太多线程会有上下文切换开销，一般 4-8 个足够。CPU 密集型任务开再多也没用（受 GIL 限制）。

- **executor.map 保持顺序**：结果顺序和输入顺序一致，即使后面的任务先完成。

---

## Application：在 RAG/Agent/架构中的位置

- **RAG 数据摄取**：批量处理文档（切分、嵌入）时用线程池加速
- **批量 Embedding**：虽然是 HTTP 请求（更适合 async），但 ThreadPoolExecutor 也能用
- **Agent 工具并发**：多个独立工具可以并发执行，缩短响应时间
- **生产服务**：文件预处理、日志分析、批量导入导出
...
总耗时: 2 秒（ceil(5/4) = 2 批）
```

### 代码结构
```
┌─────────────────────────────────────┐
│  count(files, count_lines)          │
│  ┌───────────────────────────────┐  │
│  │ partial(count_one, count_lines)│  │
│  └───────────────┬───────────────┘  │
│                  ▼                   │
│  ┌───────────────────────────────┐  │
...
- ThreadPoolExecutor + executor.map 是文件 IO 并发的标准模式
- partial 用于固定多参数函数的部分参数
- 批量处理：try-except 放循环内，单个失败不影响整体
- 耗时计算：ceil(任务数 / workers) × 单任务时间

### 🔍 索引区（记关键词）

- `concurrent.futures` - 并发模块
- `functools.partial` - 固定参数
- `max_workers` - 控制并发数
- GIL - Python 线程的限制（CPU 密集型任务受限）

### 🗑️ 垃圾区（查文档）

- executor.submit vs executor.map 的区别
- as_completed 用法
- ProcessPoolExecutor（多进程版本）
- 线程安全的数据结构
...
Month 1 Week 4 Day 4 - CLI 综合项目：测试覆盖

## 今日核心目标

为 CLI 工具编写系统化测试，覆盖正常、异常、边界三类场景

------

## Why：不学会导致的工程死穴

没有测试覆盖的 CLI 工具就像没有刹车的车——改一行代码都心惊胆战。具体表现：

- **重构恐惧**：不敢动代码，怕改出 bug
- **回归盲区**：修了 A 功能，不知道 B 功能有没有被影响
- **边界遗漏**：空文件、不存在的文件这类场景上线才发现
...
| 类型                   | 本质                 | 设计思路                               |
| ---------------------- | -------------------- | -------------------------------------- |
| **正常（Happy Path）** | 用户按预期使用       | 功能核心路径，必须通过                 |
| **异常（Error Path）** | 系统应优雅处理的错误 | 外部依赖失败（文件不存在、网络超时等） |
| **边界（Edge Case）**  | 极端但合法的输入     | 空值、零值、超长输入、特殊字符         |

**设计顺序**：先想清楚测什么（场景矩阵），再写怎么测（代码）
...
def test_count_lines_normal(tmp_path):
    # 1. Arrange - 准备测试数据
    f = tmp_path / "test.txt"
    f.write_text("line1\nline2\nline3\n")
...
- **stderr vs stdout**：typer 的 `typer.echo(..., err=True)` 输出到 stderr，但 typer 的 CliRunner 不支持 `mix_stderr=False` 参数，所以对于错误输出只能断言 exit_code，无法直接断言错误信息内容
- **包未安装**：测试文件 import 项目模块报错 `ModuleNotFoundError`，原因是没有执行 `uv pip install -e .` 把包安装到环境
- **pytest 未添加**：`import pytest` 报错，需要先 `uv add pytest --dev`
...
CLI 测试能力
    │
    ├─► Month 2: LLMClient 单元测试（Mock LLM 响应）
    │
    ├─► Month 4: RAG 评测回归（检索效果不退步）
    │
    └─► Month 5: Agent 工具测试（Mock 工具执行）
```

测试三分类思维适用于所有层级：API 测试、集成测试、端到端测试。
...
│  命令   │    正常    │    异常    │    边界    │
├─────────┼────────────┼────────────┼────────────┤
│  count  │ 正常文件   │ 文件不存在 │ 空文件     │
│  find   │ 关键词存在 │ 文件不存在 │ 关键词不存在│
└─────────┴────────────┴────────────┴────────────┘
                    │
                    ▼
Step 2: 写测试代码（每个格子一个测试函数）
                    │
                    ▼
Step 3: 运行验证
┌─────────────────────────────────────────────────┐
│  $ uv run pytest -v                             │
│  test_count.py::test_count_lines_normal PASSED  │
│  test_count.py::test_count_file_not_found PASSED│
│  ...                                            │
└─────────────────────────────────────────────────┘
```
...
- CliRunner 的具体 API 参数
- pytest 的各种 fixture 用法细节
- assert 的各种写法

**🔍 索引区（记关键词）**

- `typer.testing.CliRunner` - CLI 测试入口
- `tmp_path` - pytest 临时文件 fixture
- `result.exit_code` / `result.stdout` - 断言目标
