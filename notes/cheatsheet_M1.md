...
- 网络层 → 请求没到服务器（超时、连接失败、DNS失败）
- API服务层 → 服务器返回了错误码（401/403/429/500）
- 响应内容层 → 返回200但内容是坏的（JSON错误、字段缺失）
...
- 网络层 → 全部可重试
- API服务层 → 4xx看具体：429可重试，401/403不可重试；5xx可重试
- 响应内容层 → 有限重试（最多1次），可能是prompt问题

------

**常见状态码速查**

- 401 → 认证失败（API KEY错误）❌不可重试
- 403 → 无权限 ❌不可重试
- 429 → 限流 ✅可重试（间隔更长）
- 500 → 服务器错误 ✅可重试
